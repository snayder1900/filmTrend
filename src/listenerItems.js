...
contenedor.addEventListener('click', async(e) => {
	if (e.target.closest('.main__media')) {
        ...
        <img src="https://image.tmdb.org/t/p/w500/${resultado.backdrop_path}" class="media__backdrop-image"/>
      </div>
      <div class="media__imagen">
        <img src="https://image.tmdb.org/t/p/w500/${resultado.poster_path}" class="media__poster"/>
      </div>
      <div class="media__info">
          <h1 class="media__titulo">${resultado.title}</h1>
          <p class="media__fecha">${resultado.release_date}</p>
          <p class="media__overview">${resultado.overview}</p>
					<div class="media__button">
						<button class="media__watched button-active">add to Watched</button>
						<button class="media__queue">add to queue</button>
					</div class="media__button">
      </div>
